...
kruskal<-alpha%>% 
  mutate(statid=paste0(week, "_",sampletype))%>%
  group_by(statid)%>%
  summarise(stat=list(kruskal.test(InvSimpson,treatment)))%>%
  mutate(stat=map(.$stat,~ as.data.frame(.[["p.value"]])))%>%
  unnest_longer(stat,names_repair="minimal",simplify=F)

cecumalphastats<-alpha%>% 
  filter(sampletype=="cecum")%>%
  group_by(week)%>%
  summarise(stat=list(pairwise.wilcox.test(InvSimpson,treatment, p.adjust="fdr")))%>%
  mutate(stat=map(.$stat,~ as.data.frame(.[["p.value"]])))%>%
...
write.csv(kruskal,"Analysis/alpha_diversity_kruskal.csv",row.names=F)
write.csv(fecesalphastats,"Analysis/alpha_diversity_feces_wilcox_fdr.csv",row.names=F)
write.csv(cecumalphastats,"Analysis/alpha_diversity_cecum_wilcox_fdr.csv",row.names=F)
...
write.csv(betastats, "Analysis/beta_diversity_pairwise_adonis_fdr.csv",row.names=F)
...
write.xlsx(pbetastats,file="Analysis/beta_diversity_Prism.xlsx", sheetName = "Pairwise.Adonis_FDR",append=T, row.names=F)
...
ggplot(alpha%>%filter(sampletype %in% c("feces","cecum")),aes(x=treatment,y=InvSimpson))+
  ...
  facet_wrap(sampletype~week,ncol=4)+
    theme(legend.position="none",
        ...
        stat_pvalue_manual(filter(rbind(cecumalphastats%>%mutate(sampletype="cecum"),fecesalphastats%>%mutate(sampletype="feces")),p<0.05), 
                           label = "p.signif", 
...
for (i in c("cecum","feces")){
  data<-alpha%>%filter(sampletype==i)
  if(i=="cecum"){w=70/45 
  alphastats=cecumalphastats}
  if(i=="feces"){w=30/45 
  alphastats=fecesalphastats}
plot<-ggplot(data,aes(x=treatment,y=InvSimpson))+
  ...
  theme_bw()+
  labs(y="Alpha diversity (Inverse Simpson)")+
  scale_y_continuous(expand=c(0,w*3))+
  scale_x_discrete(labels=c("Cefoperazone Only","*C. difficile* Only","*L. acidophilus* + *C. difficile*","*L. gasseri* + *C. difficile*"),guide=guide_axis(angle=45))+
  scale_fill_manual(values=c("#205174","#A5162A","#FCE3E4","#99CEC7"))+
  facet_wrap(~week,ncol=2)+
    theme(legend.position="none",
        ...
        if(nrow(filter(alphastats,p<0.05)>0)){ plot+stat_pvalue_manual(filter(alphastats,p<0.05), 
                           label = "p.signif", 
                           y.position = max(data$InvSimpson)*1.1,
                           step.increase=.15,
                           size=3,
                           step.group.by = c("week"))
        }else(plot)
